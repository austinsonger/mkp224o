...
CC= gcc
CSTD= -std=c99 -Wall -Wextra -Wno-maybe-uninitialized -Wno-unused-function -Wmissing-prototypes -Wstrict-prototypes
CFLAGS= $(CSTD) -O3 -march=native -fomit-frame-pointer  -DED25519_donna -fPIE  -DSTATISTICS -DPASSPHRASE -DVERSION='"git-5172c0fd71740ca0b11da8149a2575dcf331d7ab"'
ASFLAGS= -fPIE
LDFLAGS= 
MV= mv
...
MAIN_EXE= $(patsubst %,%,$(MAIN_TGT))
UTIL_EXE= $(patsubst %,%,$(UTIL_TGT))
TEST_EXE= $(patsubst %,%,$(TEST_TGT))
...
calcest: $(UTIL_CALCEST_OBJ)
	$(CC) $(LDFLAGS) $(CFLAGS) -o $@.tmp $^ $(UTIL_CALCEST_LIB) && $(MV) $@.tmp $@

test_base64: $(TEST_BASE64_OBJ)
	$(CC) $(LDFLAGS) $(CFLAGS) -o $@.tmp $^ && $(MV) $@.tmp $@

test_base32: $(TEST_BASE32_OBJ)
	$(CC) $(LDFLAGS) $(CFLAGS) -o $@.tmp $^ && $(MV) $@.tmp $@

test_base16: $(TEST_BASE16_OBJ)
	$(CC) $(LDFLAGS) $(CFLAGS) -o $@.tmp $^ && $(MV) $@.tmp $@

test_ed25519: $(TEST_ED25519_OBJ)
...
depend:
	...
	cd "." && makedepend -Y -fGNUmakefile.in -o.c.o -- $(CSTD) $(ED25519_DEFS) -- $(ALL_C)

VPATH=.

%.c.o: CFLAGS += \
	-D'CRYPTO_NAMESPACETOP=crypto_sign_ed25519_donna'               \
	-D'_CRYPTO_NAMESPACETOP=_crypto_sign_ed25519_donna'             \
	-D'CRYPTO_NAMESPACE(name)=crypto_sign_ed25519_donna_\#\#name'   \
	-D'_CRYPTO_NAMESPACE(name)=_crypto_sign_ed25519_donna_\#\#name' \
